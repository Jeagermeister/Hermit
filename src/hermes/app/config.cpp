...
ConfigResult<void> overlay_env(Config& config, const EnvLookup& lookup) {
  if (const auto value = lookup("HERMES_SANDBOX_ROOT")) {
    if (value->empty()) {
      return std::unexpected(
          ConfigProblem{ConfigError::BadValue, "HERMES_SANDBOX_ROOT is set but empty"});
    }
    ...
    if (!root.is_absolute()) {
      return std::unexpected(ConfigProblem{
          ConfigError::BadValue,
          "HERMES_SANDBOX_ROOT must be an absolute path (got: " + *value +
  ...
  if (const auto value = lookup("HERMES_MODEL")) {
    if (value->empty()) {
      return std::unexpected(ConfigProblem{ConfigError::BadValue, "HERMES_MODEL is set but empty"});
    }
    config.model = *value;
    config.set_origin(Field::Model, ConfigSource::Environment);
  }

  if (const auto value = lookup("HERMES_OLLAMA_URL")) {
    if (value->empty()) {
      return std::unexpected(
          ConfigProblem{ConfigError::BadValue, "HERMES_OLLAMA_URL is set but empty"});
    }
    config.client.base_url = *value;
    config.set_origin(Field::BaseUrl, ConfigSource::Environment);
  }

  if (const auto value = lookup("HERMES_MAX_NUM_CTX")) {
    const auto parsed = parse_whole(*value, "HERMES_MAX_NUM_CTX");
...
ConfigResult<void> validate(const Config& config, const Requirements& required) {
  if (required.sandbox_root && config.sandbox_root.empty()) {
    return std::unexpected(ConfigProblem{
        ...
        "no sandbox root. Pass --root DIR, set HERMES_SANDBOX_ROOT, or put \"sandbox_root\" in a "
        "config file. There is deliberately no default: an inferred root is R1's original bug."});
  }
  if (required.model && config.model.empty()) {
    return std::unexpected(ConfigProblem{
        ConfigError::MissingRequired,
        "no model. Pass --model NAME, set HERMES_MODEL, or put \"model\" in a config file."});
  }
...
std::string Config::render() const {
  ...
  out << "config:\n";
  // Tagged like everything else: --config and HERMES_CONFIG are not interchangeable
...
ConfigResult<std::optional<std::string>> select_config_file(std::span<const std::string_view> args,
  ...
  if (const auto from_env = lookup("HERMES_CONFIG")) {
    // Set-but-empty is an error, exactly as it is for HERMES_MODEL and the rest.
    // `export HERMES_CONFIG=` is somebody trying to say something, and this is the
    ...
    if (!std::filesystem::path{*from_env}.is_absolute()) {
      return std::unexpected(ConfigProblem{
          ConfigError::BadValue,
          "HERMES_CONFIG must be an absolute path (got: " + *from_env +
...
ConfigResult<Config> load(std::span<const std::string_view> args, const Requirements& required,
  ...
  // environment and the flags. A flag beats HERMES_CONFIG for the same reason.
  const auto from_flag = find_config_flag(args);
  if (!from_flag) return std::unexpected(from_flag.error());
