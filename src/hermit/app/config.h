...
//     or `HERMIT_CONFIG` -- and never otherwise. Walking up from the working directory
//     looking for `.hermit.json` is the same act as inferring a project root, and it
...
struct Requirements {
  ...
  /// socket, so an `HERMIT_OLLAMA_URL` exported for some other tool must not break pure
  /// path resolution — and must not stop `hermit config` from printing, since that
...
/// Overlay the environment. Deliberately a short list -- `HERMIT_SANDBOX_ROOT`,
/// `HERMIT_MODEL`, `HERMIT_OLLAMA_URL`, `HERMIT_MAX_NUM_CTX` -- covering the values
...
///
/// `HERMIT_SANDBOX_ROOT` must be absolute. Unlike a flag it is not typed at launch and
...
/// Which config file to read, if any: `--config` if present, otherwise `HERMIT_CONFIG`.
///
...
/// `getenv` call made the decision untestable. `HERMIT_CONFIG=` set-but-empty is an
