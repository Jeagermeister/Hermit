...
// lowest target that links **both** `hermit_core` (for ToolSpec, RawArgs, ToolOutput)
// and `hermit_ollama` (which brings nlohmann), so putting the rendering here needs no
// new link edge and keeps `hermit_ollama` free of any dependency on `core`. `app` can
